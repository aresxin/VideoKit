...
public class VideoCreater: NSObject {
  
  // MARK: --------------------------------------------------------------- Singleton properties ------------------------------------------------------------
  
  open class var current: VideoCreater {
    struct Static {
      static var instance = VideoCreater()
    }
  ...
  open func generate(withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType, _ progress: @escaping ((Progress) -> Void), outcome: @escaping (Result<URL, Error>) -> Void) {
    ...
    dispatchQueueGenerate.async { [weak self] in
      ...
      VideoCreater.current.setup(withImages: _images, andAudios: _audios, andType: _type)
      
      /// define the input and output size of the video which will be generated by taking the first image's size
      if let firstImage = VideoCreater.current.images.first {
        VideoCreater.current.minSize = firstImage.size
      }
      
      let inputSize = VideoCreater.current.minSize
      let outputSize = VideoCreater.current.minSize
      
      self?.getTempVideoFileUrl { (videoOutputURL) in
        do {
          /// try to create an asset writer for videos pointing to the video url
          try VideoCreater.current.videoWriter = AVAssetWriter(outputURL: videoOutputURL, fileType: AVFileType.mp4)
        } catch {
          VideoCreater.current.videoWriter = nil
        ...
        if let videoWriter = VideoCreater.current.videoWriter {
          ...
          if videoWriter.startWriting() {
            ...
            videoWriterInput.requestMediaDataWhenReady(on: media_queue, using: { () -> Void in
              ...
              let numImages = VideoCreater.current.images.count
              var frameCount = 0
              var elapsedTime: Double = 0
              
              /// calculate the frame duration by dividing the full video duration by the number of images and rounding up the number
              let frameDuration = CMTime(seconds: ceil(Double(VideoCreater.current.duration / Double(VideoCreater.current.images.count))), preferredTimescale: 1)
              let currentProgress = Progress(totalUnitCount: Int64(VideoCreater.current.images.count))
              
              /// declare a temporary array to hold all as of yet unused images
              var remainingPhotos = [UIImage](VideoCreater.current.images)
              ...
              while (videoWriterInput.isReadyForMoreMediaData && frameCount < numImages) {
                ...
                if VideoCreater.current.type == .single {
                  ...
                  imageForVideo = VideoCreater.current.images[frameCount]
                  
                  if VideoCreater.current.type == .multiple {
                    ...
                    elapsedTime += VideoCreater.current.audioDurations[frameCount] <= 1 ? VideoCreater.current.minSingleVideoDuration : VideoCreater.current.audioDurations[frameCount]
                  } else {
                    ...
                    let audio_Time = VideoCreater.current.audioDurations[0]
                    let total_Images = VideoCreater.current.images.count
                    elapsedTime += audio_Time / Double(total_Images)
                  }
                }
                
                /// append the image to the pixel buffer at the right start time
                if !VideoCreater.current.appendPixelBufferForImage(imageForVideo, pixelBufferAdaptor: pixelBufferAdaptor, presentationTime: nextStartTimeForFrame) {
                  DispatchQueue.main.async {
                    VideoCreater.current.videoWriter = nil
                    outcome(.failure(VideoCreaterError(error: .kFailedToAppendPixelBufferError)))
                  }
                }
              ...
              if let _maxLength = VideoCreater.maxVideoLengthInSeconds {
              ...
              videoWriter.finishWriting { () -> Void in
                if self?.audioURLs.isEmpty == true {
                  if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
                    let newPath = URL(fileURLWithPath: documentsPath).appendingPathComponent("\(VideoCreater.fileName).m4v")
                  ...
                  VideoCreater.current.mergeAudio(withVideoURL: videoOutputURL) { result in
                    switch result {
            ...
            DispatchQueue.main.async {
              VideoCreater.current.videoWriter = nil
              outcome(.failure(VideoCreaterError(error: .kFailedToStartAssetWriterError)))
            }
          }
        } else {
          DispatchQueue.main.async {
  ...
  open class func mergeMovies(videoURLs: [URL], outcome: @escaping (Result<URL, Error>) -> Void) {
    ...
    guard !_videoURLs.isEmpty else {
      DispatchQueue.main.async {
        outcome(.failure(VideoCreaterError(error: .kMissingVideoURLs)))
      }
      return
    }
    ...
    if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
      ...
      completeMoviePath = URL(fileURLWithPath: documentsPath).appendingPathComponent("\(VideoCreater.fileName).m4v")
      ...
      DispatchQueue.main.async {
        outcome(.failure(VideoCreaterError(error: .kFailedToFetchDirectory)))
      }
    }
    ...
    if let completeMoviePath = completeMoviePath {
      ...
      if let videoTrack: AVMutableCompositionTrack = composition.addMutableTrack(withMediaType: AVMediaType.video, preferredTrackID: kCMPersistentTrackID_Invalid), let audioTrack: AVMutableCompositionTrack = composition.addMutableTrack(withMediaType: AVMediaType.audio, preferredTrackID: kCMPersistentTrackID_Invalid) {
        ...
        if let exportSession = AVAssetExportSession(asset: composition, presetName: presetName ?? AVAssetExportPresetHighestQuality) {
          ...
          DispatchQueue.main.async {
            outcome(.failure(VideoCreaterError(error: .kFailedToStartAssetExportSession)))
          }
        }
      }
    }
  ...
  open func reverseVideo(fromVideo videoURL: URL, outcome: @escaping (Result<URL, Error>) -> Void) {
    self.reverseVideoClip(videoURL: videoURL, andFileName: VideoCreater.fileName) { (result) in
      switch result {
      case .success(let url):
        outcome(.success(url))
      case .failure(let error):
  ...
  open func splitVideo(withURL videoURL: URL, atStartTime start: Double? = nil, andEndTime end: Double? = nil, outcome: @escaping (Result<URL, Error>) -> Void) {
    if start != nil {
      guard start! >= 0.0 else {
        DispatchQueue.main.async {
          outcome(.failure(VideoCreaterError(error: .kFailedToReadStartTime)))
        }
        return
      }
    }
    ...
    if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
      let outputURL = URL(fileURLWithPath: documentsPath).appendingPathComponent("\(VideoCreater.fileName).m4v")
      ...
      if let exportSession = AVAssetExportSession(asset: sourceAsset, presetName: VideoCreater.presetName ?? AVAssetExportPresetHighestQuality) {
  ...
  open func mergeVideoWithAudio(videoUrl: URL, audioUrl: URL, outcome: @escaping (Result<URL, Error>) -> Void) {
    ...
    if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
      ...
      if let exportSession = AVAssetExportSession(asset: mixComposition, presetName: VideoCreater.presetName ?? AVAssetExportPresetHighestQuality) {
        exportSession.outputURL = outputURL
        exportSession.outputFileType = AVFileType.mp4
        exportSession.shouldOptimizeForNetworkUse = true
  ...
  fileprivate func setup(withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType) {
    ...
    if self.type == .single {
      if let _image = VideoCreater.shouldOptimiseImageForVideo ? _images.first?.resizeImageToVideoSize() : _images.first {
      ...
      for _image in _images {
        autoreleasepool {
          if let imageData = _image.scaleImageToSize(newSize: CGSize(width: VideoCreater.videoImageWidthForMultipleVideoGeneration, height: VideoCreater.videoImageWidthForMultipleVideoGeneration))?.pngData() {
    ...
    duration = max((audioURLs.isEmpty ? VideoCreater.videoDurationInSeconds : _duration), minVideoDuration)
    
    if audioURLs.isEmpty {
      audioDurations = [Double](repeating: duration / Double(images.count), count: images.count)
    }
    
    if let _scaleWidth = VideoCreater.scaleWidth {
    ...
    dispatchQueueMerge.async { [weak self] in
      ...
      if let videoTrack = videoAsset.tracks(withMediaType: .video).first {
        ...
        for (index, audioUrl) in self?.audioURLs.enumerated() ?? [].enumerated() {
          ...
          let audioTimeRange = CMTimeRange(start: CMTime.zero, duration: VideoCreater.maxVideoLengthInSeconds != nil ? audioDuration : audioAsset.duration)
          
          let shouldAddAudioTrack = VideoCreater.maxVideoLengthInSeconds != nil ? audioDuration.seconds > 0 : true
          ...
          duration = duration + (VideoCreater.maxVideoLengthInSeconds != nil ? audioDuration : audioAsset.duration)
        }
        ...
        if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
          ...
          /// create a path to the video file
          let videoOutputURL = URL(fileURLWithPath: documentsPath).appendingPathComponent("\(VideoCreater.fileName).m4v")
        ...
        DispatchQueue.main.async {
          outcome(.failure(VideoCreaterError(error: .kFailedToReadVideoTrack)))
        }
      }
    }
  }
  ...
  private func reverseVideoClip(videoURL: URL, andFileName fileName: String?, outcome: @escaping (Result<URL, Error>) -> Void) {
    ...
    media_queue.async {
      ...
      if !videoURL.absoluteString.contains(".DS_Store") && acceptableVideoExtensions.contains(videoURL.pathExtension) {
              ...
              if let assetVideoTrack = videoAsset.tracks(withMediaType: .video).first {
                ...
                if reader.startReading() {
                  ...
                  if timesSamples.count > 1 {
                    ...
                    DispatchQueue.main.async {
                      outcome(.failure(VideoCreaterError(error: .kFailedToReadProvidedClip)))
                    }
                  }
                } else {
                  DispatchQueue.main.async {
                    outcome(.failure(VideoCreaterError(error: .kFailedToStartReader)))
                  }
                }
              }
            } catch {
        ...
        DispatchQueue.main.async {
          outcome(.failure(VideoCreaterError(error: .kUnsupportedVideoType)))
        }
      }
    }
  }
  ...
  private func fillPixelBufferFromImage(_ image: UIImage, pixelBuffer: CVPixelBuffer) {
    ...
    if let context = CGContext(data: pixelData, width: Int(contextSize.width), height: Int(contextSize.height), bitsPerComponent: 8, bytesPerRow: CVPixelBufferGetBytesPerRow(pixelBuffer), space: rgbColorSpace, bitmapInfo: CGImageAlphaInfo.noneSkipFirst.rawValue) {
      ...
      context.setFillColor(type == .single ? UIColor.black.cgColor : VideoCreater.videoBackgroundColor.cgColor)
